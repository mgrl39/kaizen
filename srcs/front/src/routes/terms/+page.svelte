<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      const res = await fetch(`/docs/${currentLang}/terms.md`);
      const raw = await res.text();
      content = await marked(raw);
    } catch (err) {
      content = `<p class="text-danger">${currentLang === 'es' ? 'Error al cargar los términos y condiciones.' : 'Error loading terms and conditions.'}</p>`;
    } finally {
      loading = false;
    }
  });
